...
`codegraph:fs` cho 3 method này, vì chúng thao tác trên session đã tồn tại,
không phải hành động gốc (dial SSH / mở project). Việc **tạo** session
(`SSHStartTerminal`, `CodeGraphStartTerminal`) vẫn gate bởi capability của
module sở hữu (`ssh:connect`, `codegraph:fs`) như bình thường — sau khi tạo
