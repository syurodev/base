...
|---|---|---|---|
| `vault.status` | `read` | `builtin.vault` / `vault:manage` / `vault` | Chỉ trạng thái (`locked`/`unlocked`/`not_configured`) |
| `vault.search` | `read` | `builtin.search` / `vault:search` / `item:search` | Cross-type metadata search (`VaultSearch`), không decrypt payload |

**Không expose qua MCP, bất kể grant**: `SetupVault`, `UnlockVault`,
`UnlockVaultWithRecovery`, `LockVault`. Master Password là bí mật con người
nhập — cho agent gọi được coi như xóa bỏ lý do có `VaultGate`. Đây là loại trừ
cứng, không phải thứ mở lại được bằng cách nới policy.
...
| SSH | `ssh-profile` | `ssh-secret` | [SSH](/dev-kit/ssh) |
| Database | `db-profile` | `db-password` | [Database](/dev-kit/database) |
| Kafka (design) | `kafka-profile` | `kafka-secret` | [Kafka UI](/dev-kit/kafka) |
...
<Cards>
  <Card title="SSH" href="/dev-kit/ssh" description="Thin adapter ssh-profile trên vaultitem" />
  <Card title="Notes" href="/dev-kit/notes" description="Thin adapter đầu tiên trên vaultitem" />
  <Card title="Database" href="/dev-kit/database" description="Thin adapter db-profile trên vaultitem" />
  <Card title="Kafka UI" href="/dev-kit/kafka" description="Design — kafka-profile trên vaultitem" />
