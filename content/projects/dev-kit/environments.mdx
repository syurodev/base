...
**Known gap (khớp code):** `DeleteTag` **chưa** chặn “tag đang được profile
tham chiếu”. SSH/DB chỉ validate `Exists` lúc **create** profile. Khi wire MCP
confirmation gate, reader phải fail-closed nếu tên env trên profile không
resolve được (`requiresConfirmation = true`). Thêm cross-service in-use check
khi UX cần chặn xóa.
