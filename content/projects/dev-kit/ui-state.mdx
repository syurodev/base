...
```mermaid
flowchart LR
  Store["Zustand store\n(vd useSidebarStore)"] -->|"persist middleware"| Adapter["UI state storage adapter\n(getItem/setItem/removeItem)"]
  Adapter -->|"debounce ~300-500ms"| Bridge["UIStateGet / UIStateSet"]
  Bridge --> GW["Capability Gateway\nbuiltin.uistate"]
  GW --> SVC["internal/uistate.Service"]
  SVC --> DB[("ui_state — internal/storage,\nkhông thuộc vault_items")]
```
...
```sql
ui_state(key TEXT PRIMARY KEY, value TEXT /* JSON */, updated_at)
```
...
```ts
persist(useSidebarStore, { name: 'sidebar', storage: uiStateStorage })
```
