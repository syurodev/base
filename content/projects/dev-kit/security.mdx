...
thông tin để giải mã lại (`internal/crypto/envelope.go`). Đúng 5 field:

- `version` (luôn `2`) / `algorithm` (`xchacha20-poly1305`): đọc đúng cách kể
  cả khi sau này đổi thuật toán.
- `key_id`: định danh khoá dùng cho field đó, vd `note-body`.
