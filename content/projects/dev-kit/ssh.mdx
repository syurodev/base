...
metadata `ssh-profile`. `SSHCreateProfile` phải reject nếu thiếu `env`. Giá
trị `env` là tên 1 tag trong registry [Environments](/dev-kit/environments)
(`dev`/`stg`/`prod` mặc định, tạo thêm tùy chỉnh được). `ssh.runCommand` qua
MCP tra `requiresConfirmation` của tag đó trước khi chạy — nếu bật, phải xác
nhận tương tác mỗi lần gọi, không có `allow_always`. `env=prod` luôn bật
(khóa cứng); `dev`/`stg` mặc định tắt nhưng bật lên được — đáng cân nhắc bật
sẵn cho SSH vì đây là tool rủi ro cao nhất trong toàn bộ MCP tool surface
hiện có, dù không bắt buộc như `prod`. Chi tiết cơ chế:
