...
`env` là bắt buộc: `SSHCreateProfile` reject nếu thiếu hoặc tên tag không tồn tại trong registry [Environments](/dev-kit/environments) (qua `EnvLookup.Exists(name)`, `internal/ssh/ssh.go`).
...
**"Hỏi quyền theo policy env tag"**: field bắt buộc `env` trên
metadata `ssh-profile` đã **Implemented** (xem Vaultitem contract ở trên).
Phần còn design: `ssh.runCommand` qua MCP tra `requiresConfirmation` của tag
đó trước khi chạy — chưa có vì chưa có MCP runtime để đọc field này. Khi có,
thiết kế là: nếu bật, phải xác nhận tương tác mỗi lần gọi, không có
`allow_always`; `env=prod` luôn bật (khóa cứng, đã đúng ở tầng
`internal/environments.Service`); `dev`/`stg` mặc định tắt nhưng bật lên
được — đáng cân nhắc bật sẵn cho SSH vì đây là tool rủi ro cao nhất trong
toàn bộ MCP tool surface hiện có, dù không bắt buộc như `prod`. Chi tiết cơ
chế: [MCP / Agent § `env` bắt buộc](/dev-kit/mcp-agent). `ssh.listProfiles`/
