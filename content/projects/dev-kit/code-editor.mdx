...
type CodeEditorProps = {
  ...
  extensions?: Extension[]; // consumer tự mang: sql(...), java(), json(), yaml(), markdown()
  readOnly?: boolean;
};
```
...
new EditorView({
  state: EditorState.create({
    doc: value,
    extensions: [/* basicSetup, ... */, language.of(initialExtensions)],
  }),
});

// mỗi lần prop `extensions` đổi (effect trong React wrapper)
view.dispatch({ effects: language.reconfigure(newExtensions) });
```
