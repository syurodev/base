...
| ID | Requirement |
|---|---|
| FR-1 | User tạo được connection profile Kafka (bootstrap servers, security protocol, SASL mechanism, credential, **`env` bắt buộc** — `dev`/`stg`/`prod`) — lưu qua vaultitem, cùng pattern SSH/Database. Lưu được **nhiều** profile cùng lúc. |
...
| Metadata (plaintext) | `name`, `bootstrap_servers` (list), `security_protocol` (`PLAINTEXT`/`SSL`/`SASL_PLAINTEXT`/`SASL_SSL`), `sasl_mechanism` (`PLAIN`/`SCRAM-SHA-256`/`SCRAM-SHA-512`), `username`, **`env`** (`dev`\|`stg`\|`prod`, **bắt buộc**) |
...
| `kafka.listTopics` | `read` | `builtin.kafka` / `kafka:connect` / `connection` | Thao tác trên cluster đang active; nếu chưa có → trigger popup chọn, xem flow ở trên. Hỏi quyền theo policy env tag (xem dưới) |
| `kafka.describeTopic` | `read` | `builtin.kafka` / `kafka:connect` / `connection` | Input: `topic`. Hỏi quyền theo policy env tag (xem dưới) |
| `kafka.peekMessages` | `read` | `builtin.kafka` / `kafka:connect` / `connection` | Input: `topic`, `partition`, `limit?`. Giới hạn số message mặc định (NFR-4). Hỏi quyền theo policy env tag (xem dưới) |
| `kafka.listConsumerGroups` | `read` | `builtin.kafka` / `kafka:connect` / `connection` | Hỏi quyền theo policy env tag (xem dưới) |
| `kafka.publishMessage` | `execute` (`destructiveHint`) | `builtin.kafka` / `kafka:produce` / `connection` | Input: `topic`, `key?`, `payload`, `headers?`, `partition?`. Output: `{partition, offset, timestamp}`. Cluster đích luôn là cluster đang active — agent không truyền `profileId`. Hỏi quyền theo policy env tag (xem dưới) |
...
**"Hỏi quyền theo policy env tag"**: vì `env` là field bắt buộc trên mọi profile (FR-1), 5 tool ở trên tra `requiresConfirmation` của tag đó ([Environments](/dev-kit/environments)) trước khi chạy — nếu tag đó bật xác nhận, call phải qua xác nhận tương tác mỗi lần, kể cả `kafka.listTopics`/`describeTopic`/`peekMessages`/`listConsumerGroups` dù chỉ là `read`, không có `allow_always`. `env=prod` **luôn** bật (khóa cứng, không tắt được); `dev`/`stg` mặc định tắt nhưng bật lên được. Chi tiết cơ chế: [MCP / Agent § `env` bắt buộc](/dev-kit/mcp-agent). `kafka.listProfiles`/`kafka.connectionStatus` **không** bị luật này — chỉ đọc metadata/status, không chạm cluster thật.
