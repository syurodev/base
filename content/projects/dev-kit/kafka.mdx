...
| `kafka.listTopics` | `read` | `builtin.kafka` / `kafka:connect` / `connection` | Thao tác trên cluster đang active; nếu chưa có → trigger popup chọn, xem flow ở trên. Luôn hỏi quyền trước khi chạy (xem dưới) |
| `kafka.describeTopic` | `read` | `builtin.kafka` / `kafka:connect` / `connection` | Input: `topic`. Luôn hỏi quyền trước khi chạy |
| `kafka.peekMessages` | `read` | `builtin.kafka` / `kafka:connect` / `connection` | Input: `topic`, `partition`, `limit?`. Giới hạn số message mặc định (NFR-4). Luôn hỏi quyền trước khi chạy |
| `kafka.listConsumerGroups` | `read` | `builtin.kafka` / `kafka:connect` / `connection` | Luôn hỏi quyền trước khi chạy |
| `kafka.publishMessage` | `execute` (`destructiveHint`) | `builtin.kafka` / `kafka:produce` / `connection` | Input: `topic`, `key?`, `payload`, `headers?`, `partition?`. Output: `{partition, offset, timestamp}`. Cluster đích luôn là cluster đang active — agent không truyền `profileId`. Luôn hỏi quyền trước khi chạy |
...
**"Luôn hỏi quyền trước khi chạy"**: vì `env` là field bắt buộc trên mọi profile (FR-1), 5 tool ở trên **luôn** yêu cầu xác nhận tương tác mỗi lần gọi — kể cả `kafka.listTopics`/`describeTopic`/`peekMessages`/`listConsumerGroups` dù chỉ là `read`. Không có standing grant hay `allow_always` nào bỏ qua được bước này. Chi tiết cơ chế + lý do: [MCP / Agent § `env` bắt buộc](/dev-kit/mcp-agent). `kafka.listProfiles`/`kafka.connectionStatus` **không** bị luật này — chỉ đọc metadata/status, không chạm cluster thật.
