...
auto-approve. UI đối chiếu Figma page 18 (2026-09-09): rail 1 cây, tab
Topics/Publish/Consumer groups, inspector message, dialog New cluster
(`frontend/src/modules/kafka/`). Xem [MCP / Agent](/dev-kit/mcp-agent) và
[Implementation status](/dev-kit/implementation-status).

**Phạm vi Implemented là v1 = FR-1…FR-9.** FR-10…FR-18 (nhiều connection,
Avro, reset offset, batch, topic admin, member breakdown, auto-refresh) mới ở
mức **Designed** — có màn hình trong Figma (section design-only, page 18),
**chưa có bridge/Go**; runtime hiện giữ đúng 1 connection sống. Các mục
dưới đây ghi cả 2 tầng, phần Designed được đánh dấu rõ.
</Callout>
...
FR-1…FR-9: Implemented. FR-10…FR-18: Designed — mô tả dưới là thiết kế đích,
không phải hành vi hiện tại (FR-2 hiện chỉ giữ 1 connection; connect B đóng A).
...
### Bridge & capability (Implemented — v1, 12 method)

Đúng như `internal/bridge/kafka.go` hiện tại. 7 method thao tác trên cluster
đang online dùng scope cố định `connection` — không nhận `profileId`, vì chỉ có
1 connection sống; `KafkaDisconnect` vì thế không có tham số.
...
| `KafkaListTopics` | `kafka:connect` | `connection` |
| `KafkaDescribeTopic` | `kafka:connect` | `connection` |
| `KafkaPeekMessages` | `kafka:connect` | `connection` |
| `KafkaListConsumerGroups` | `kafka:connect` | `connection` |
| `KafkaPublishMessage` | `kafka:produce` | `connection` |

Dữ liệu trả về cũng ở mức v1: `TopicDetail` chỉ có tên, replication factor và
danh sách partition ID; `ConsumerGroupInfo` chỉ có group ID + tổng lag;
`PublishRequest` chưa có `headers` (`internal/kafka/broker.go`). UI vì thế
không vẽ số liệu mà bridge không trả (không có state/member của group, không
có retention).

**Designed — chưa có trong bridge (FR-10…FR-18):**
...
Khi FR-10 vào code, capability tách thành 3: `kafka:connect` (quản lý profile + vòng đời connection + đọc dữ liệu cluster), `kafka:produce` (publish, kể cả batch), và `kafka:admin` **mới** (topic CRUD + reset offset — hành động thay đổi cấu trúc/state cluster, khác hẳn publish 1 message hay chỉ đọc). Tách riêng để có thể cấp `kafka:connect`+`kafka:produce` mà **không** cấp `kafka:admin` — dev thường xuyên publish message test nhưng hiếm khi cần tạo/xoá topic.

**Sẽ đổi so với v1** (theo FR-10): mọi method (trừ `KafkaListProfiles`/`KafkaCreateProfile`/`KafkaDeleteProfile`/`KafkaTestConnection`) nhận `profileId` tường minh, scope `profile:<id>` thay vì scope cố định `connection` — vì nhiều connection sống cùng lúc thì không còn 1 "connection đang online" duy nhất để ngầm định.

`KafkaConnect`/`KafkaDisconnect` **là bridge method, chỉ UI gọi được** (từ KafkaView hoặc sidebar, hoặc gián tiếp từ popup ở flow "Agent gọi khi chưa có cluster active"). Agent không tự chọn cluster. `KafkaCreateTopic`/`KafkaDeleteTopic`/`KafkaAlterTopicConfig`/`KafkaResetConsumerGroupOffset` (Designed) — xem quyết định MCP exposure ở mục dưới.

### MCP tool surface

Xem cơ chế chung + rationale ở [MCP / Agent](/dev-kit/mcp-agent). 7 tool đầu
là Implemented (`internal/mcp/tools_kafka.go`); `kafka.publishBatch` và
`kafka.createTopic` là Designed, cùng đợt với bridge ở trên.
...
| `kafka.peekMessages` | `read` | `builtin.kafka` / `kafka:connect` / `profile:active` | Input: `topic`, `partition`, `limit?`. Giới hạn số message mặc định (NFR-4). Payload trả raw/JSON; auto-decode Avro (FR-12) là Designed. Hỏi quyền theo policy env tag (xem dưới) |
| `kafka.listConsumerGroups` | `read` | `builtin.kafka` / `kafka:connect` / `profile:active` | Group ID + tổng lag; breakdown member (FR-17) là Designed. Hỏi quyền theo policy env tag (xem dưới) |
| `kafka.publishMessage` | `execute` (`destructiveHint`) | `builtin.kafka` / `kafka:produce` / `profile:active` | Input: `topic`, `key?`, `payload`, `partition?` (`headers?` Designed). Output: `{partition, offset, timestamp}`. Cluster đích luôn là cluster đang active — agent không truyền `profileId`. Hỏi quyền theo policy env tag (xem dưới) |
| `kafka.publishBatch` **(Designed)** | `execute` (`destructiveHint`) | `builtin.kafka` / `kafka:produce` / `profile:active` | Input: `topic`, `messages[]`. Output: mảng kết quả best-effort (FR-15). Hỏi quyền theo policy env tag (xem dưới) |
| `kafka.createTopic` **(Designed)** | `execute` | `builtin.kafka` / `kafka:admin` / `profile:active` | Additive, không phá dữ liệu có sẵn — expose được. Hỏi quyền theo policy env tag (xem dưới) |
...
- **Chưa decode schema nào** — peek/publish luôn raw/JSON. Avro là Designed (FR-12/13); Protobuf ngoài phạm vi cả ở bản thiết kế.
