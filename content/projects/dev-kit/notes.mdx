...
**Tab đang mở note bị trash nơi khác — đã chốt: hiện trạng thái, không tự
đóng.** Body đang trên màn hình và vẫn là của người đọc để copy ra; một tab
biến mất dưới tay họ tệ hơn. Banner nói rõ note đang ở trash, sửa vẫn lưu, và
có nút Restore ngay đó. Figma `Notes / 03b Editor — Note trashed elsewhere`.
...
("Note details") đang làm với `NoteMetaPanel`. Figma đã thiết kế xong —
`Notes / 05 Editor — Version history` và `Notes / History Panel Content` /
`Notes / History Diff Content` — với đúng hai con số ở trên: *"checkpoints
every idle 5 min · keeps last 50"*.
...
Note, folder **và version** sync như mọi `vault_items` record
(`record_type IN (note, note-folder, note-version)`), không có pipeline riêng
— xem [Data & sync](/dev-kit/data-sync).

<Callout type="warn" title="Danh sách type ở trên là mô tả, không phải bộ lọc">
Không có chỗ nào trong tầng sync lọc theo record type.
`SQLMutationRepository.Put` đặt `SyncState` local rồi ghi operation cho **bất
kỳ** type nào qua được `IsSupportedItemType` — xem
`internal/vaultitem/sqlmutationrepository.go`. Nên khoảnh khắc `note-version`
được thêm vào switch đó là version sync, không cần wire gì thêm; và ngược lại,
muốn một type **không** sync thì phải viết một bộ lọc hôm nay chưa tồn tại.
Đây là lý do query history của Database nằm ở bảng cục bộ riêng chứ không phải
một item type.
</Callout>

**Version sync — đã chốt.** Lịch sử sửa đổi đi theo note sang máy khác. Cái
giá là băng thông: mỗi checkpoint là một full snapshot, giữ 50 bản, nên một
note 50KB có thể đẩy tới 2.5MB qua sync. Chấp nhận vì lịch sử mà chỉ tồn tại
trên một máy thì mất ý nghĩa ngay khi người dùng đổi máy — khác với query
history của Database, thứ gắn với một thiết bị theo đúng nghĩa.
...
Sáu đợt, tất cả đã land:

1. **Trash** — `deleted_at` là field trên chính note, không phải record type
   mới, nên restore trả note về đúng folder/tag/color cũ. Sweep chạy piggyback
   trong `List` nên mở Dashboard là đủ già hoá trash, không cần vào màn Trash.
2. **Version history** — `note-version` + `ListByTypeAndMetaField` đẩy xuống
   `json_extract`. Checkpoint no-op khi không đổi; restore tự chụp
   `pre-restore` trước khi ghi đè.
3. **Backlink** — bảng `note_links` (migration m19), index ghi như side-effect
   của Create/Update khi body còn plaintext. Dạng serialize là
   `[[note:<id>]]`, vì body là markdown nên inline void phải sống sót qua text.
4. **Sidebar + Trash screen** — sidebar đăng ký qua `useSidebarContent`. Không
   phải di chuyển mà là **viết lại**: `SidebarMenuButton` gọi `useSidebar()` và
   throw nếu không có `SidebarProvider`, nên primitive shadcn không sống được
   trong rail của shell. Notes là module cuối cùng còn tự vẽ rail.
5. **Panel history + backlink + trạng thái trashed** — diff dùng LCS theo dòng,
   không so từng dòng: chèn một dòng ở đầu sẽ khiến mọi dòng dưới bị đánh dấu
   vừa xoá vừa thêm, và một lịch sử tô sáng cả note thì vô nghĩa.
6. **`[[` note link** — inline void đầu tiên của editor. `LinkKit` là inline
   nhưng bọc text thật, `ImageUpload` là void nhưng block-level, nên schema,
   render và round trip đều wiring từ đầu.
...
- Version history lưu full snapshot mỗi checkpoint (không diff) — tốn dung
  lượng vault **và băng thông sync** theo số version × kích thước note;
  idle-checkpoint 5 phút và giới hạn 50 version chưa cấu hình được theo user.
