...
> Trạng thái: **Partial**. Backend Phase A và Gateway MVC đã có runtime trong
> `/Users/syuro/Workspace/PERSONAL/dev-kit`; desktop PKCE và rotating refresh
> token đã có runtime trong `/Users/syuro/Workspace/dev-kit`. Basic E2E trên
> deployment thật đã pass ngày 2026-08-10: login, session, one push, pull và
> apply. Ma trận multi-device/retry/revoke vẫn chưa hoàn tất.
...
- backend Phase A và Go production-transport contract E2E đã có; commit
  `d54a72f` sửa contract cho namespaced client idempotency key;
...
> Nhắc nhở tiến trình: BE Phase A, Gateway và desktop auth đã có runtime; phải
> chạy đủ ma trận trên trước khi coi Phase 3 là **Implemented**.
...
- **Desktop auth milestone — Basic E2E pass**: Authorization Code + PKCE,
  refresh/logout, OS-keychain refresh-token rotation và không còn nhập bearer
  thủ công.
- **BE Phase B — Sau**: device management UI, operational revocation flows,
