...
> runtime trong `/Users/syuro/Workspace/PERSONAL/dev-kit/dev-kit-backend` (commit
...
- **BE Phase B (device revoke) — Design approved**: self-service
  `GET/POST /v1/sync/devices` list+revoke, last-ACTIVE guard, audit,
  Redis denylist TTL 60s (`sync:revoked-device:{sub}:{deviceId}`) do API ghi /
  Gateway đọc (fail-open nếu Redis down), desktop bridge tối thiểu + G4 dùng
  API. UI polish / Keycloak logout / retention / multi-region vẫn out of scope.
  Spec: `docs/superpowers/specs/2026-08-11-be-phase-b-device-revoke-design.md`
