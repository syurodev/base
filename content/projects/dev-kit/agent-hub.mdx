...
Agent Hub là module built-in tương lai giúp DevKit trở thành nơi tập trung chạy nhiều AI coding agent (Claude Code, Codex, Gemini CLI, ...) song song, mỗi agent một "tab" độc lập, quản lý qua Capability Gateway như mọi module khác. Trên nền đó, module mở thêm khả năng **agent-gọi-agent**: chọn 1 agent làm "master", cấp cho nó 1 tool để tự gọi sang agent khác lấy kết quả — không cần con người điều phối thủ công.

import { Callout } from 'fumadocs-ui/components/callout';

<Callout type="warn" title="Design only — Phase 4 postponed pending sandbox merge">
Trang này là design cho một module chưa có runtime trong DevKit checkout hiện tại.
Không có `internal/agenthub` hay `internal/acp` tại thời điểm viết. **Implementation
plan đã viết nhưng đang hoãn**: spawn agent CLI mà không có OS sandbox là quá nguy
hiểm (Gateway/permission UI không chặn bash/fs nội bộ của agent). Stack sandbox đã
**chốt** ([ADR-009](/dev-kit/architecture-decisions#adr-009-os-primitive-sandbox-for-agent-hub-vendor-processes)):
Linux bubblewrap (+ Landlock best-effort), macOS Seatbelt — package
`internal/sandbox` **Implemented trên desktop branch `cursor/agent-sandbox-a5ea`**
(bwrap + Landlock helper + Seatbelt); merge + verification trước khi resume Hub.
Claude adapter `Open()` phải spawn qua `sandbox.DefaultRunner().Start(…)`. External
plugins / marketplace vẫn Phase 5–6. Một phần thiết kế ACP đã được xác thực bằng
thực nghiệm thật — các chỗ đó được đánh dấu rõ. Trạng thái implementation thật
nằm ở [Implementation status](/dev-kit/implementation-status).
</Callout>
...
flowchart TD
  ...
  REG --> A1[Claude adapter]
  REG --> A2[Codex adapter - chưa viết]
...
- **Vendor adapter registry** — `map[AgentType]VendorAdapter`. Mỗi adapter chịu trách nhiệm:
  1. Biết cách spawn agent tương ứng (binary path hoặc `npx <package>`, biến môi trường auth cần thiết).
...
AgentSession {
  ...
  projectPath      string
  status           enum        // idle | running | awaiting_permission | error | done
  parentSessionId  string?     // ai đã spawn/gọi session này
  depth            int         // parentSessionId == null → 0
  title            string?     // lấy từ session_info_update nếu vendor hỗ trợ
  createdAt        timestamp
}
...
TurnRecord {
  sessionId        string
  stopReason       string
  usage            NormalizedUsage   // cost, tokens — từ vendor adapter
  rawMeta          json              // toàn bộ _meta chưa parse, giữ nguyên cho audit
}
```
...
- Chỉ có adapter cho Claude ở thời điểm viết (do đã có trace thật để dựa vào); Codex/Gemini cần viết thêm theo cùng interface `VendorAdapter` khi có nhu cầu — không phải rework core.
