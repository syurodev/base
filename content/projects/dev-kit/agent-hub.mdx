...
Trang này là design cho một module chưa có runtime trong DevKit checkout hiện tại. Không có `internal/agenthub` hay `internal/acp` tại thời điểm viết. Một phần thiết kế trong trang này đã được xác thực bằng thực nghiệm thật (chạy `@agentclientprotocol/claude-agent-acp` qua ACP, bắt log JSON-RPC thật) — các chỗ đó được đánh dấu rõ, phần còn lại vẫn là suy luận từ spec. Trạng thái implementation thật nằm ở [Implementation status](/dev-kit/implementation-status).
</Callout>
