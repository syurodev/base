...
### T0 — Đo trước, đừng xây

Hai phép đo rẻ, không cam kết thiết kế nào, và cả hai đều nhằm tránh lặp lại việc suy ra hành vi từ việc đọc bundle của vendor.

- **Log `UnknownMethod`.** `claudeConnHandler.UnknownMethod` trong `internal/agenthub/adapter_claude.go` hiện trả lỗi và **không ghi gì** — comment của `internal/acp/handler.go` còn lấy đúng `fs/*` làm ví dụ cho method chưa hỗ trợ. Nghĩa là nếu agent từng gọi `fs/read_text_file`, DevKit đã âm thầm trả lỗi mà không ai biết. Thêm hook báo cáo giống `OnUnknownUpdate`, rồi chạy vài session thật. Đây là câu trả lời dứt điểm cho câu hỏi `fs/*` bên dưới, tốn vài dòng.
- **Capture session có subagent.** `TestClaudeACPCapture` đã dựng sẵn để làm việc này; chỉ cần một prompt buộc agent spawn subagent. Kết quả ghi thẳng vào `testdata/`, và là input cho T2.
