...
- **Prompt không chỉ có text — chưa làm, và không phải vì khó.** `acp.PromptRequest` đã model `[]ContentBlock`, nhưng `SessionRuntime.Prompt` nhận `string`. Đổi chữ ký lúc này là xây cho một caller chưa tồn tại: `AgentComposer` có prop `attachedFiles`/`contextFiles`/`onSelectContext` nhưng **không gì trong `AgentHubView` nuôi chúng** — UI vẽ nút đính kèm mà chưa nối cả hai đầu. Đây là một lát dọc (nguồn file → state composer → bridge → Go → ACP), nên làm cả lát hoặc chưa làm, đừng làm nửa dưới.
