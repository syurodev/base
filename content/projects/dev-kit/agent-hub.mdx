...
<Callout type="warn" title="Design only — next Phase 4 (trước plugins)">
Trang này là design cho một module chưa có runtime trong DevKit checkout hiện tại.
Không có `internal/agenthub` hay `internal/acp` tại thời điểm viết. Theo
[Roadmap](/dev-kit/roadmap), **Agent Hub là ưu tiên tiếp theo (Phase 4)**;
external plugins / marketplace dời xuống Phase 5–6. Một phần thiết kế đã được
xác thực bằng thực nghiệm thật (chạy `@agentclientprotocol/claude-agent-acp` qua
ACP, bắt log JSON-RPC thật) — các chỗ đó được đánh dấu rõ, phần còn lại vẫn là
suy luận từ spec. Trạng thái implementation thật nằm ở
