...
<Callout type="info" title="Partial — v1 surface shipped (mock + Claude)">
Runtime đã có trên desktop branch Agent Hub: `internal/acp`, `internal/agenthub`
(mock + Claude adapters), bridge/UI multi-tab, MCP tools `agenthub.*`, và vendor
spawn qua `sandbox.DefaultRunner().StartPiped(...)` (fail-closed nếu thiếu
backend). **Codex / Gemini adapters vẫn Design.** Chưa claim production-verified
end-to-end với Claude Code thật trong CI — smoke trên desktop. Sandbox API:
[/dev-kit/sandbox](/dev-kit/sandbox) (`Start` = capture helpers; `StartPiped` =
ACP stdio). Trạng thái: [Implementation status](/dev-kit/implementation-status).
</Callout>
