...
- **Prompt không chỉ có text (Implemented).** Cả lát dọc: picker file native (`AgentHubChooseFiles`) → "Browse files…" đầu menu context → state đính kèm theo từng session trong `AgentHubView` → `InvokeInput.Attachments` → `buildPrompt` → `[]acp.ContentBlock`. `SessionRuntime.Prompt` nhận block thay vì `string`; `acp.ContentBlock` mở rộng theo đúng schema SDK sinh ra (`text`, `image{data,mimeType}`, `resource_link{name,uri}`, `resource{resource{uri,text|blob,mimeType}}`).
