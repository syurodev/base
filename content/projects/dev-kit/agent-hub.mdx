...
<Callout type="warn" title="Design only — Phase 4 postponed; sandbox Implemented">
Trang này là design cho module chưa có runtime `internal/agenthub` / `internal/acp`.
**Sandbox OS đã Implemented** — xem [/dev-kit/sandbox](/dev-kit/sandbox) và
[ADR-009](/dev-kit/architecture-decisions#adr-009-os-primitive-sandbox-for-agent-hub-vendor-processes)
(`internal/sandbox` trên desktop `main`, PR #22). Hub vẫn hoãn cho tới khi
adapter vendor spawn qua `sandbox.DefaultRunner().Start(...)` (fail-closed nếu
thiếu backend). External plugins / marketplace vẫn Phase 5–6. Trạng thái:
