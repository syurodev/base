---
title: Module tabs
description: Tab strip — cơ chế mở/đóng/focus nhiều tab, kéo-thả, preview/pin kiểu VS Code. Phase 5 đã xong — một store chia partition theo module, strip opt-in theo module, render trong content card.
---

# Module tabs

Cơ chế mở/đóng/focus nhiều tab cùng lúc: kéo-thả sắp xếp, preview tab kiểu VS
Code (single-click, thay double-click mới ghim), nút đóng. Từ 2026-09-09
(nhánh `phase-5-workbench-ui`, commit `e11eebb`) tab **thuộc về module**:
`frontend/src/stores/workbenchTabs.ts` giữ một `modules: Record<moduleId,
ModuleTabs>` — mỗi partition có `openTabs`/`activeTabId`/`tabMru` riêng,
`moduleOfTab(id)` suy module từ tiền tố id — và `activeModuleId` chọn
partition đang hiện. `Workbench.tsx` chỉ vẽ `TabStrip` khi
`ModuleManifest.tabs === true` (hôm nay chỉ Notes bật); module khác không có
strip nào cả. Strip render ở mép trên **content card** (`h-12` = 48px, khớp
Figma `116:1346`), title bar 40px chỉ còn ba vùng icon. Chuyển module qua Dock
giữ nguyên partition cũ; quay lại thì `focusModule` sticky theo `activeTabId`
gần nhất, chưa có tab mới mở preview "home" của module.

import { Callout } from 'fumadocs-ui/components/callout';
import { Cards, Card } from 'fumadocs-ui/components/card';

<Callout type="info" title="Phase 5 per-module ownership — đã implement">
[Title bar](/dev-kit/title-bar) chỉ còn 3 vùng icon+tooltip; strip ở content
card; store chia partition theo module và strip opt-in qua
`ModuleManifest.tabs`. Một khác biệt so với bản design ở cuối trang: thay vì
N store riêng, đây là **một** store Zustand chia partition — cùng cô lập
(partition A không biết partition B), nhưng `AgentSessionsSubscriber` và
`TabSwitcher` vẫn đọc được qua `moduleTabs(modules, id)` /
`selectActiveModuleTabs`. Guard: `frontend/src/stores/workbenchTabs.test.ts`.
Cơ chế tương tác (`TabStrip`, `Reorder`, `openTab`/`pinTab`/`preview`) giữ
nguyên.
</Callout>
...
<Callout type="info" title="Đã implement (2026-09-09)">
Mục này là bản design gốc, giữ lại để đọc lý do. Runtime: một store chia
partition theo module (`modules` + `activeModuleId`, xem đầu trang) thay vì
N store riêng; opt-in qua `ModuleManifest.tabs`; sticky-per-module trong
`Workbench.tsx` `focusModule`. Điều hướng chéo module vẫn là non-goal.
</Callout>
