...
(`frontend/src/workbench/Workbench.tsx`) đây vẫn là **1 tab strip toàn cục**
trộn tab của mọi module vào 1 mảng phẳng — nhưng nó **không còn nằm trong
title bar**. `TabStrip` render ở mép trên của **content card** (`h-12` = 48px,
khớp Figma `116:1346` content-surface: `tab-strip` 0..48 rồi `content-slot`
bên dưới, cùng một surface). Title bar 40px chỉ còn ba vùng icon. Nói cách
khác, **nửa đầu của cuộc migrate Phase 5 đã xảy ra** (đổi nơi render); nửa
sau (store theo module, opt-in theo module) là phần bên dưới mô tả.

import { Callout } from 'fumadocs-ui/components/callout';
import { Cards, Card } from 'fumadocs-ui/components/card';

<Callout type="warn" title="Design — Phase 5 chuyển sang per-module ownership">
[Title bar](/dev-kit/title-bar) đã bỏ tab-strip khỏi title bar (chỉ còn 3
vùng icon+tooltip: status / module quick action / app quick action) — **phần
này đã implement**, strip giờ ở content card. Phần **chưa** implement: tab
chuyển thành của module — mỗi module tự quản store tab riêng thay vì 1
`useWorkbenchTabsStore` toàn cục, và module không cần tab thì không có strip. Chi tiết ở mục
