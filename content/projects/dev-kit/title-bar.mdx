...
<Callout type="warn" title="Design only — supersedes ADR-012 tab-strip">
Trang này mô tả hướng thiết kế mới cho Phase 5 Workbench UI, **thay thế hoàn
toàn** mô hình multi-tab của [Workbench tabs](/dev-kit/workbench-tabs)
(`TabStrip`, `Reorder.Group`, `openTab`/`pinTab`/`preview`). Model mới: **chỉ 1
module active tại 1 thời điểm** — không còn nhiều tab mở song song trong title
bar. Chuyển module qua [Dock](/dev-kit/shell-primitives), không qua tab
click/drag-reorder nữa. Chưa có runtime — `frontend/src/workbench/Workbench.tsx`
hiện tại vẫn chạy theo `workbench-tabs.mdx`.
</Callout>
...
- **Thay thế**: [Workbench tabs](/dev-kit/workbench-tabs) — toàn bộ phần
  `TabStrip`/`Reorder`/multi-tab không còn áp dụng trong hướng thiết kế mới.
...
<Cards>
  <Card title="Workbench tabs" href="/dev-kit/workbench-tabs" description="Model TabStrip cũ — bị supersede bởi trang này" />
