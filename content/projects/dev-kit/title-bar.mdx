...
```mermaid
flowchart LR
  TL["Traffic lights<br/>(macOS native)"] --> SB["Sidebar toggle<br/>(cố định, ngoài 3 vùng)"]
  SB --> ST["Status area<br/>(trái)"]
  ST --> MA["Module quick actions<br/>(giữa)"]
  MA --> APP["App quick actions<br/>(phải)"]
```

import { Callout } from 'fumadocs-ui/components/callout';

<Callout type="warn" title="Design only — sẽ supersede ADR-012 (tab strip trong title bar) khi được chốt">
...
runtime — `frontend/src/workbench/Workbench.tsx` hiện tại vẫn chạy theo
[ADR-012](/dev-kit/architecture-decisions), nên ADR-012 vẫn `Accepted +
Implemented` cho tới khi thiết kế này được chốt thành ADR riêng.
</Callout>

## Sidebar toggle (cố định, ngoài 3 vùng)

Nút icon riêng, vị trí cố định ngay sau traffic lights — **không thuộc**
status area hay bất kỳ vùng nào trong 3 vùng bên dưới. **Luôn hiển thị trong
phạm vi Workbench** (không opt-in theo module, khác module quick action
area): mọi module đều có sidebar, không có trường hợp module không cần
sidebar nên ẩn nút. Trước khi vault `unlocked` (chưa có Workbench, xem
[Vault](/dev-kit/vault)) nút này không xuất hiện — chi tiết ở
[Sidebar § Toggle](/dev-kit/sidebar).
...
primitive riêng — chi tiết đầy đủ ở [Sidebar](/dev-kit/sidebar).
...
| Panel-bottom | Bật/tắt [bottom slot](/dev-kit/shell-primitives#bottom-slot-primitive) |

**Toggle bottom slot nằm ở vùng 3, không nằm ngoài 3 vùng như sidebar toggle.**
Sidebar toggle bị ghim sát traffic light vì App Shell trong Figma đặt nó ở đó;
toggle này không có ràng buộc đó, nên đặt ở cuối vùng 3 — hai toggle layout kẹp
hai đầu title bar. **Luôn hiện và luôn bấm được**, kể cả khi module đang mở
không đóng góp nội dung nào: nó bật/tắt cái khay, và khay tự giải thích khi
rỗng. Ẩn/disable theo module sẽ làm nút nhấp nháy mỗi lần chuyển module.
...
- **Điều hướng module (Dock)**: `Workbench.tsx` render dock (xem
  ...
  hiện tại).
- **Sidebar content primitive**: [Sidebar](/dev-kit/sidebar) —
  `useSidebarContent`, toggle global, resize, ràng buộc vault unlocked.
...
<Cards>
  ...
  <Card title="Sidebar" href="/dev-kit/sidebar" description="useSidebarContent — primitive thứ 3, nội dung theo module active" />
