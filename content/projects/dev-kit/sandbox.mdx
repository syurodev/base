...
xem [Agent Hub](/dev-kit/agent-hub) (Partial) và
...
// Capture-and-wait (stdout/stderr buffered) — helpers / one-shot commands.
proc, err := r.Start(ctx, sandbox.Policy{
    WorkDir: "/abs/path/to/project",
    Network: sandbox.NetworkNone, // or NetworkHost
    Env:     map[string]string{"PATH": "/usr/bin:/bin"},
}, sandbox.Cmd{Path: "/usr/bin/tool", Args: []string{"--once"}})

// Interactive ACP / vendor agent stdio — prefer StartPiped.
piped, err := r.StartPiped(ctx, sandbox.Policy{
    WorkDir: "/abs/path/to/project",
    Network: sandbox.NetworkHost, // agent may need net for model API
    Env:     map[string]string{"PATH": "/usr/bin:/bin"},
}, sandbox.Cmd{Path: "/usr/bin/agent", Args: []string{"--acp"}})
// piped.StdinWriter() / piped.StdoutReader() for line-delimited JSON-RPC
```
...
<Cards>
  ...
  <Card title="Agent Hub" href="/dev-kit/agent-hub" description="Partial consumer — Claude vendor spawn uses StartPiped" />
