...
| `db.query` | `execute` (`destructiveHint`) | `builtin.database` / `db:query` / `profile:active` | Không nhận `profileId` — chạy trên **active profile cho agent**. Không phân biệt SELECT/UPDATE — coi mọi query là có thể phá dữ liệu. Luôn hỏi quyền trước khi chạy (xem dưới) |

**"Luôn hỏi quyền trước khi chạy"**: cần thêm field bắt buộc `env`
(`dev`/`stg`/`prod`) vào metadata `db-profile` — không có trong bảng
Vaultitem contract hiện tại (vì đó là mô tả runtime Postgres đã Implemented;
`env` là field mới, design only). `DBCreateProfile` phải reject nếu thiếu
`env`. Vì `env` bắt buộc trên mọi profile, `db.query` qua MCP **luôn** yêu cầu
xác nhận tương tác mỗi lần gọi, không có `allow_always`. Chi tiết cơ chế:
