...
`Env` là bắt buộc: `DBCreateProfile` reject nếu thiếu hoặc tên tag không tồn tại trong registry [Environments](/dev-kit/environments) (qua `EnvLookup.Exists(name)`, `internal/database/database.go`).
...
**"Hỏi quyền theo policy env tag"**: field bắt buộc `Env` trên metadata
`db-profile` đã **Implemented** (xem Vaultitem contract ở trên). Phần còn
design: `db.query` qua MCP tra `requiresConfirmation` của tag đó trước khi
chạy — chưa có vì chưa có MCP runtime để đọc field này. Khi có, thiết kế là:
nếu bật, phải xác nhận tương tác mỗi lần gọi, không có `allow_always`;
`env=prod` luôn bật (khóa cứng, đã đúng ở tầng `internal/environments.Service`);
`dev`/`stg` mặc định tắt nhưng bật lên được. Chi tiết cơ chế:
