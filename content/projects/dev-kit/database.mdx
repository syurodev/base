...
| `DBSetActiveProfile` | `db:query` | `profile:<id>` | |
| `DBActiveProfile` | `db:query` | `profile:active` | |
...
| `DBListActivity` | `db:query` | `profile:<id>` | snapshot session đang chạy, không phải time-series |
| `DBCancelQuery` | `db:query` | `session:<pid>` | huỷ statement của 1 pid — human-only |
| `DBTerminateSession` | `db:query` | `session:<pid>` | đóng connection của 1 pid — human-only |
| `DBExplain` | `db:query` | `profile:<id>` | lấy kế hoạch, **không** chạy câu lệnh |
| `DBExplainAnalyze` | `db:query` | `profile:<id>` | **chạy thật** trong transaction rồi rollback — theo policy env tag |
...
- Profile `engine=mariadb` connect thành công tới MariaDB 10.x thật bằng cùng
  driver `mysql`, không cần code path riêng. **Đã kiểm chứng** với MariaDB
  10.11 thật: `driver_mariadb_live_test.go` chạy cả bảy method của `Conn`,
  `IndexLister` phân biệt đúng unique với non-unique, và lời hứa rollback của
  `ExplainAnalyze` — test bị skip trừ khi trỏ vào một server qua
  `DEVKIT_MARIADB_ADDR`.
...
- `EXPLAIN ANALYZE` trên **MySQL** không có xương sống chuẩn hoá, chỉ có cây
  text trong `Raw`. MariaDB thì đầy đủ nhờ `ANALYZE FORMAT=JSON`.
...
<Callout type="info" title="Core Implemented">
`QueryPlanner` trong `internal/database/inspector.go`, implement ở
`inspector_postgres.go` và `inspector_mysql_plan.go`, bridge
`DBExplain`/`DBExplainAnalyze`. Cả ba engine đều có `EXPLAIN`; phần `ANALYZE`
thì MySQL khác hai engine kia — xem ngay dưới.
</Callout>
...
| MySQL 8 | `EXPLAIN FORMAT=JSON`; `EXPLAIN ANALYZE` | `query_block`; bản ANALYZE **chỉ có text dạng cây** |
...
**Cả hai engine đều implement `QueryPlanner`, nhưng chỉ một bên dựng được xương
sống ở bước `ANALYZE`.** `EXPLAIN FORMAT=JSON` trả cây `query_block` trên cả
MySQL lẫn MariaDB nên phần `Explain` dựng giống hệt nhau. Chỗ rẽ là `ANALYZE`:
`ANALYZE FORMAT=JSON` của MariaDB mang theo `r_rows` và `r_total_time_ms` bên
cạnh số ước lượng, còn `EXPLAIN ANALYZE` của MySQL chỉ trả **cây dạng text**.
MariaDB điền đầy xương sống; MySQL trả cây đó vào `Raw` — đúng công dụng của
`Raw`, và plan viewer đã có sẵn tab *Raw Output* cho nó.

Điều thay đổi so với v1 là quyết định **để trống cả interface**, không phải
nhận định về cây text: parse nó thành node vẫn sẽ khẳng định một độ chính xác
mà output không có.

Parser đi qua JSON theo cách tổng quát chứ không liệt kê từng khoá của engine:
`nested_loop` và các khoá tương tự là **container**, bảng bên trong nằm ở cùng
độ sâu với block chứa chúng — nếu không, mỗi phép join lại đẻ thêm một tầng ma.
Cả hai cách viết khoá số dòng đều được đọc, vì chỉ đọc cách của MariaDB thì mọi
plan MySQL sẽ báo 0 dòng, trông như một câu trả lời thật thay vì một chỗ thiếu.
...
4. Trả về **danh sách tách biệt** với FK thật — ERD vẽ nét đứt, mỏng hơn và mờ
   hơn, không trộn chung một kiểu đường với FK thật.
...
Phần dựng editor (theme, key bindings, wiring React) thuộc về
[Code editor](/dev-kit/code-editor). Trang này chỉ nói phần **Database cần cung
cấp**: SQL dialect theo `engine` + schema map cho completion source của
`@codemirror/lang-sql` — không lặp lại chi tiết editor ở đây.
</Callout>

### Bảng kết quả: chuột phải để lấy dữ liệu ra

Chuột phải một ô cho *Copy cell*, *Copy row as JSON*, *Copy column name* và
*Inspect cell*; chuột phải header cột cho tên cột và **toàn bộ giá trị của cột**,
nối bằng xuống dòng — dạng dán thẳng được vào mệnh đề `IN` hoặc bảng tính.

Ba ràng buộc, đều là chuyện đúng/sai chứ không phải tiện nghi:

**Copy lấy giá trị thật, không lấy thứ đang hiển thị.** Ô bảng cắt ở 280px với
`truncate`, nên cái mắt đọc được thường không phải cái nằm trong dòng. Copy mà
đọc text trên DOM là đưa cho người ta một giá trị cụt mà không báo gì.

**Nhãn copy cột đếm số dòng đang có, không viết "copy cột".** Kết quả bị cap ở
200 dòng; menu còn hiện thêm cảnh báo khi `truncated`. Đưa 200 dòng của một
tổng số không biết dưới chữ "cột" là thứ dễ đi thẳng vào một `WHERE` sai.

**`rowAsJSON` rơi về `col_N` cho cột không tên.** `SELECT 1, 1` trả về hai cột
không tên; thiếu fallback thì hai giá trị đè lên cùng một khoá và một cái biến
mất im lặng — JSON vẫn hợp lệ, vẫn parse được, chỉ thiếu dữ liệu. Cùng lý do
đó, header khoá theo cả tên lẫn chỉ số: `SELECT a.id, b.id` trả về hai cột cùng
tên `id` và React gộp chúng lại.

<Callout type="warn" title="Clipboard đi qua runtime, không qua navigator">
`navigator.clipboard` không đáng tin trong WKWebView và hỏng **im lặng** —
người dùng bấm copy, không lỗi, không có gì trong clipboard. Mọi lệnh copy đi
qua `Clipboard.SetText` của `@wailsio/runtime`, gói trong
`frontend/src/bridge/clipboard.ts`.
</Callout>
...
## Sidebar: nhóm connection theo folder

Folder trong sidebar là **nhóm hiển thị thuần tuý**, không phải một thực thể.
Nó sống trong `frontend/src/modules/database/databaseStore.ts` dưới dạng
`folders: Record<tên folder, profileId[]>` cùng `expandedFolders`, và persist
qua `ui_state` với key `database.folders`. Không có bản ghi nào trong
`vault_items`, không có method bridge nào cho folder.

Đó là chủ ý, không phải chưa làm tới. `vault_items` **sync-bound by
construction**: `SQLMutationRepository.Put` ghi mutation intent trong cùng
transaction, nên mọi thứ đặt vào đó đều sinh sync traffic. Một cách sắp xếp
cây cho dễ nhìn trên **máy này** không phải dữ liệu người dùng cần mang sang
máy khác — và nếu ép vào, nó còn cần một item type mới chỉ để chứa một
preference, đi kèm validator riêng vì `decodeMetadataObject` chạy
`DisallowUnknownFields`.

### Chuột phải trên các dòng sidebar

Dòng bảng trong cây schema là lý do đáng làm nhất: chèn `SELECT * FROM x LIMIT
50` trước đó **chỉ có double-click** và không có dấu hiệu nào cho biết điều đó
tồn tại. Menu gọi tên hành động và vẫn hiện phím tắt bên cạnh, kèm *Focus in
ERD*, *Copy table name*, *Re-index schema*.

Dòng profile có *Move to folder* (submenu), *Connection options*, *Copy
connection target*, *Delete connection*. Các nút hover **giữ nguyên** dù menu
đã bao trọn chúng: gán profile vào folder là tính năng vừa mới có, biến nó
thành right-click-only là giấu nó đi lần nữa. Toolbar cộng chuột phải cùng làm
một việc là chuẩn mực, không phải trùng lặp thừa.

Dòng folder có *Edit folder*, *New folder*, *Delete folder*.

### Sửa folder: đổi tên, đổi icon, xoá

Component `DB / Folder Icon Picker` (`532:2726`) không chỉ là bộ chọn icon —
nó là dialog **Edit folder** gộp ba việc: ô *Folder name*, lưới icon, và ba nút
*Delete folder* / *Cancel* / *Save*. Trước đó folder chỉ là trang trí: hai tên đóng cứng,
`assignProfileToFolder` nằm trong store mà không component nào gọi, và không có
đường nào để tạo/đổi tên/xoá — nên mọi profile ở lại Ungrouped vĩnh viễn.

Chính màn hình đó tự ghi ranh giới của mình, nguyên văn: *"the folder icon is
cosmetic — grouping never changes how a profile connects"*. Icon **không** mang
ngữ nghĩa an toàn; env tag `prod` mới là thứ gác always-ask và safe-mode, và
một folder đeo icon khiên không được phép trông như một đảm bảo.

Hợp đồng:

1. Icon lưu **tên**, không lưu ảnh: thêm `folderIcons: Record<tên folder,
   string>` vào đúng slice `database.folders` đang persist. Không SVG inline,
   không đường dẫn file, không emoji — tên giữ `ui_state` nhỏ và để icon ăn màu
   theo theme thay vì đóng cứng màu vào ảnh.
2. Allowlist đóng, đúng 20 tên trong lưới của màn hình: `folder`, `server`,
   `laptop`, `database`, `monitor`, `layers`, `clock`, `hash`, `tag`,
   `terminal`, `user`, `sliders-horizontal`, `file-text`, `link`, `palette`,
   `plus`, `db-postgres`, `db-mysql`, `db-mariadb`, `db-sqlite`. **16 cái đầu
   là Lucide, 4 cái cuối không phải** — chúng là nhãn hiệu engine (voi
   Postgres, cá heo MySQL, hải cẩu MariaDB, lông vũ SQLite) và phải lấy từ
   asset của design system, không tra được trong `lucide-react`. Bất cứ cài đặt
   nào coi giá trị này là "tên icon Lucide" đều hỏng ở đúng 4 ô cuối.
3. Tên lạ — sửa tay `ui_state`, bản cũ, hoặc Lucide đổi tên icon ở lần nâng
   version — rơi về `folder` mặc định, không được làm vỡ render.
4. Danh sách nằm ở frontend (`folderIcons.tsx`), không qua bridge. Đây là
   preference hiển thị, Go không cần biết folder tồn tại. Bốn mark engine
   được export từ chính vector layer của Figma vào `icons/engineMarks.tsx`.
5. *Delete folder* chỉ xoá **nhóm**, profile bên trong quay về Ungrouped.
   Không có đường nào từ màn hình này dẫn tới xoá một connection thật; xoá
   profile là `DBDeleteProfile`, ở chỗ khác.
6. Figma vẽ dialog nhưng **không** vẽ cách một profile vào được folder. Chỗ đó
   là một dropdown trên chính dòng profile, đồng thời cho biết nó đang nằm ở
   đâu. Một profile thuộc tối đa một folder, nên gán là gỡ khỏi mọi folder
   khác.

<Callout type="warn" title="Ô đổi tên biến migration id thành điều kiện tiên quyết">
Folder khoá theo **id**, tên chỉ là nhãn — và điều này phải làm **trước** dialog
chứ không phải sau. Bản v0 khoá theo tên, nên đổi tên là tạo khoá mới và
profile bên trong lẫn trạng thái gập đều rơi về mặc định cùng lúc. Chừng nào
chưa có UI đổi tên thì đó là lỗi lý thuyết; màn hình này **có ô đổi tên**, nên
nó thành đường đi thường ngày.

`persist` chạy ở `version: 1` với `migrateFoldersToIds()` trong
`databaseStore.ts` — hàm thuần, export riêng để test được mà không dựng store.
Test khẳng định không mất một profile nào trên đường chuyển, vì mất ở đây là
mất im lặng: người dùng chỉ thấy một nhóm rỗng, không có lỗi nào.
</Callout>

<Callout type="info" title="db-sqlite có trong lưới, SQLite thì chưa có driver">
Lưới icon đã trù tính SQLite. `driverFor()` hiện chỉ nhận postgres, mysql và
mariadb. Icon là nhãn cosmetic nên không mâu thuẫn — một folder có thể tên
"SQLite" và rỗng — nhưng đừng đọc lưới này như danh sách engine được hỗ trợ.
</Callout>

## Trạng thái lỗi có thiết kế riêng

Bốn tình huống được cho màn hình riêng thay vì một dòng lỗi đỏ, vì mỗi cái có
một bước tiếp theo rõ ràng mà thông báo của driver không nói ra. Component ở
`frontend/src/modules/database/components/DatabaseStates.tsx` và
`DeleteProfileDialog.tsx`.

**Xoá profile.** Trước đây xoá xảy ra ngay ở cú bấm đầu tiên vào icon thùng rác,
không xác nhận ở UI và cũng không có cổng nào phía sau: `DBDeleteProfile` dùng
đúng capability và scope như chạy một query bình thường. Việc của dialog **không
phải** thêm một cú bấm, mà là **gọi tên những gì mất theo** — mật khẩu đã seal,
history cục bộ, saved query, và tư cách thành viên folder. Không thứ nào trong
đó nhìn thấy được từ một dòng sidebar, nên trước đó không ai cân nhắc được cái
giá. Số lượng history/saved là best-effort: vault khoá thì đọc không ra, và điều
đó **không được phép** chặn việc xoá — nên để trống thay vì đoán.

**Vault khoá** tách làm hai như thiết kế. Khi không đọc được gì thì thay cả vùng
nội dung. Khi vault khoá **giữa phiên** thì chỉ là một banner, vì các dòng đang
trên màn hình đã ra khỏi database **trước khi** khoá và đang nằm trong bộ nhớ —
xoá chúng đi là phá mất công việc mà cái khoá không có quyền đụng tới.

Nút *Unlock vault* dùng lại `VaultGate`: nó vốn đã biết cách mở khoá và vốn đã
đọc lại trạng thái vault, chỉ là không có lý do đọc lại sau khi mount. Module
phát một event xin nó kiểm tra lại, thay vì dựng form mở khoá thứ hai phải giữ
đồng bộ với form thứ nhất.

**TLS bị từ chối** có màn riêng vì kết nối bị chặn **trước khi** được thử —
`ErrInsecureTLSForRemoteHost`. Một lỗi driver mà người đọc không làm gì được là
sai hình dạng cho tình huống đó.

<Callout type="warn" title="Nhận diện bằng chuỗi, không phải kiểu lỗi">
Bridge trả về **thông báo**, không phải lỗi có kiểu, nên việc phân loại là so
chuỗi — gom về `errorStates.ts` với test riêng. Trước đó hai view tự so chuỗi
inline với cách viết hơi khác nhau, nên đổi một câu chữ bên Go sẽ làm mất trạng
thái ở một nửa module mà không ai biết. Nếu muốn chắc chắn hơn, đúng hướng là
cho bridge trả mã lỗi ổn định.
</Callout>
