...
`Driver` chỉ có duy nhất `Dial(ctx, ConnConfig) → Conn`. `Conn` chốt đúng 6
method, không hơn — phần việc không phải mọi engine làm được nằm ở interface
tuỳ chọn riêng, xem "Session đang chạy" bên dưới:
...
- Query plan v1 chỉ Postgres. SQLite `EXPLAIN QUERY PLAN` không có cost nên
...
Chưa có runtime. Cần 1 interface tuỳ chọn mới trên driver và 3 bridge method.
UI đã thiết kế: Figma `15 Database` → `Database / 10 Active queries`.
</Callout>

### Không phải method thứ 7 của `Conn`

Mục 1 chốt `Conn` đúng 6 method. Quyết định đó **giữ nguyên** — activity nằm ở
một interface riêng mà driver implement hoặc không:
...
`BlockedBy` là lý do chính người ta mở màn này. pgAdmin/DBeaver hiển thị danh
...
<Callout type="warn" title="Design only">
Chưa có runtime. Cần interface tuỳ chọn thứ hai trên driver và 2 bridge method.
v1 **chỉ Postgres**; MySQL/MariaDB theo sau qua cùng interface.
</Callout>
...
tuỳ chọn lần thứ hai là bằng chứng pattern đó đúng, không phải cái cớ để nới
luật "`Conn` đúng 6 method".
