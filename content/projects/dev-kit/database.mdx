...
### Sửa folder: đổi tên, đổi icon, xoá (design — chưa implement)
...
*Delete folder* / *Cancel* / *Save*. Hiện mọi folder dùng chung một cặp
`Folder`/`FolderOpen` trong `DatabaseSidebar.tsx`, nên "Production" và
"Development" chỉ khác nhau ở chữ.
...
4. Danh sách nằm ở frontend, không qua bridge. Đây là preference hiển thị, Go
   không cần biết folder tồn tại.
...
`folders`, `expandedFolders` và `folderIcons` đều khoá theo **tên folder**.
Chừng nào chưa có id ổn định, đổi tên là tạo khoá mới: profile trong folder,
trạng thái gập và icon đều rơi về mặc định cùng lúc. Trước đây đó là lỗi lý
thuyết vì UI không cho đổi tên — nhưng màn hình này **có ô đổi tên**, nên nó
trở thành đường đi thường ngày. Cho folder một id và để tên chỉ là nhãn là việc
phải làm **trước** khi dựng dialog này, không phải sau.
</Callout>
