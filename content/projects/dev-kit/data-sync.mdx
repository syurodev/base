...
## Conflict resolution (Git-style)

> Thiết kế (chưa implement). Xử lý conflict theo mô hình **Git**: **không tự chọn thắng**,
> giữ record ở trạng thái conflict với **cả hai bản** để user tự giải quyết.
...
3. UI hiển thị **ours vs theirs** (tương tự `<<<<<<< ours / ======= / >>>>>>> theirs`),
   ...
   (chỉ cho note text). **Secret không merge ngầm** — chỉ chọn một bên (đúng §10.1).
   UI đích: **3-pane merge kiểu JetBrains** (Local | Result editable | Remote, accept
   từng hunk ◀/▶); MVP dùng marker inline `<<<<<<< / ======= / >>>>>>>` để test. Item
   secret (db/ssh) rút gọn còn 2 lựa chọn ours/theirs.
...
<Cards>
  <Card title="SaaS sync backend" href="/dev-kit/saas-backend" description="Spring Boot 4 + Keycloak + PG18 + Redis" />
