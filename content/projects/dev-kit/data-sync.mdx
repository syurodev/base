...
Sync client hiện đã có queue/cursor, authenticated HTTP push/pull và remote
apply transaction. Việc tự động capture local create/update/delete của
`vault_items` vào outbox vẫn chưa triển khai; UI nói rõ giới hạn này.
...
- Chưa có production backend. Local mutation capture, entity-version allocation
  khi nhiều device offline và conflict-resolution UX thuộc Phase 3C.
