---
title: Connections
description: SSH, database và profile kết nối được bảo vệ.
---

# Connections

Module connections quản lý profile SSH/DB và metadata kết nối — secrets nằm trong vault.

```mermaid
flowchart TD
  UI[Connections UI] --> GW[Capability Gateway]
  GW --> CONN[Connection service]
  CONN --> VAULT[(Secret material in vault)]
  CONN --> SESS[SSH session / DB query]
```

## Scope (hiện tại)

- Lưu profile SSH/PostgreSQL trong vault; không hardcode secret trong config plaintext.
- SSH: test connection, trust host-key pin, chạy lệnh từ shell UI, và tạo local/remote TCP forwarding.
- PostgreSQL: query và schema browse qua connection pool với TLS verify-full mặc định cho remote host.
- Đóng/cleanup session rõ ràng sau thao tác; xóa profile hoặc shutdown app sẽ dừng các forward liên quan.

Forward listeners chỉ bind loopback, không persist qua restart, và dùng capability riêng `ssh:forward`.

## Non-goals (MVP)

- Enterprise policy phức tạp
- Chia sẻ connection cho team qua cloud plaintext
- Public-facing tunnel listener hoặc unrestricted network bind

import { Cards, Card } from 'fumadocs-ui/components/card';

<Cards>
  <Card title="Implementation status" href="/dev-kit/implementation-status" description="Evidence paths và capability scope" />
  <Card title="Architecture decisions" href="/dev-kit/architecture-decisions" description="ADR-007: TLS và host-key pinning" />
</Cards>
