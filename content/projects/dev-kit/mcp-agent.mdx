...
### `env` bắt buộc trên mọi profile — luôn hỏi quyền qua MCP, kể cả read

Mọi profile của Kafka/Database/SSH (module dùng "active target selection") có 1 field **bắt buộc** — `env` (`dev` | `stg` | `prod`) — khai báo khi tạo profile, không có giá trị rỗng/bỏ qua được. Field này tách biệt với metadata mô tả kết nối (host, port...) — mục đích duy nhất là gắn nhãn rủi ro cho từng target.

Luật gắn với `env`: **mọi MCP call (`origin = "mcp"`) thao tác trên active target đều phải qua xác nhận tương tác trước khi chạy — kể cả tool `read`, không có standing grant nào bỏ qua được, không có tùy chọn "luôn cho phép".** Vì `env` bắt buộc trên mọi profile, luật này áp dụng vô điều kiện cho nhóm tool đó, không có nhánh "target chưa gắn nhãn thì thôi".
...
- **Không có `allow_always`** — khác hẳn model đã quan sát ở `session/request_permission` của ACP (có `allow_once`/`allow_always`). Ở đây cố tình chỉ có Allow/Deny cho từng lần gọi — nếu cho `allow_always`, cluster "quên đang chọn cluster nào" sẽ quay lại đúng vấn đề mà cơ chế này sinh ra để chặn.
