...
| Agent Hub | [Agent Hub § MCP tool surface](/dev-kit/agent-hub#mcp-tool-surface) — `agenthub.listAgents/createAgentSession/invokeAgent/getSessionStatus/cancelAgent` (Partial; mock + Claude) |
...
```text
UI/CLI → McpInstall(agent, scope, projectPath?)
       → internal/mcp/install writer (claude-code | codex)
       → file config trên máy
Agent spawn → cmd/devkit-mcp (stdio) → unix socket → DevKit GUI tools
```
...
<Cards>
  ...
  <Card title="Agent Hub" href="/dev-kit/agent-hub" description="Partial — agenthub.* MCP tools (mock + Claude); Codex/Gemini Design" />
