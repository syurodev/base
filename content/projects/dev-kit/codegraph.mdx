...
- **Popover neo vào node, không phải `SplitPanel`** — thiết kế đầu đặt panel
  này vào [SplitPanel](/dev-kit/shell-primitives#split-panel-primitive), nhưng
  SplitPanel đã được dành cho **chat trực tiếp với agent**, nên không còn chỗ.
  Popover mở ngay cạnh node vừa bấm: canvas giữ nguyên vị trí/zoom (đúng lý do
  ban đầu đã loại modal), và node đang nói tới vẫn nằm trong tầm mắt — thứ
  SplitPanel không đảm bảo được vì nó đẩy canvas hẹp lại.
- **1 node → 1 popover, 2 tab** — `DETAIL` (FQN, plan note, source) và
  `COMMENTS` (xem [Comment trên plan node](#comment-trên-plan-node)).
  Không phải 2 popup riêng cho cùng 1 lần bấm.
- **Popover che mất một phần canvas** — chấp nhận có chủ đích. Đổi lại là
  không phải co canvas mỗi lần xem 1 node, và popover đóng lại là xong.
